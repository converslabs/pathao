...
class Ajax {
    ...
    private function __construct() {
		...
		public function sncue_admin(){
			wp_enqueue_script(
		'pathao-popup', plugin_dir_url(__FILE__) . 'assets/js/popup.js', ['jquery'], '1.2.0', true );
...
$raw_sandbox = false;
	...
	public function send_order_to_pathao( $maybe_order_id = null ) {
		...
		$store_id = sdevs_pathao_store_id() ?: get_option( 'pathao_store_id' );
		if ( ! $store_id ) {
			if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
...
$recipient_phone = str_replace( '+88', '', $recipient_phone );
		...
		if ( is_object( $res ) && isset( $res->success ) && ! $res->success ) {
			error_log( "[Pathao Debug] send_order_to_pathao failed: " . print_r( $res, true ) );
			if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
